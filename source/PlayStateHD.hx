...
class PlayState extends MusicBeatState
{
	...
	var shootBeatsBSide:Array<Int> = [32,36,40,44,48,52,56,64,68,72,76,80,88,94,104,108,112,116,120,128,131,133,136,138,140, 160, 163, 165, 168, 171, 173,180,184,188,196,200,204,208,216, 224, 232, 240, 248, 256];
	...
	//	FlxG.sound.cache(Paths.inst(PlayState.SONG.song));
		...
		if (curStage == 'limo' || curStage == 'date')
		add(overlay);
	...
	function noteMiss(direction:Int = 1, ?healthLoss:Float = 0.04, ?playAudio:Bool = true, ?skipInvCheck:Bool = false):Void
		{
			if (!boyfriend.stunned && !startingSong && (!boyfriend.invuln || skipInvCheck) )
			{
				...
				if (combo > 5 && dad.curCharacter == 'gf-date')
				{
					dad.playAnim('sad');
				}
				misses += 1;
